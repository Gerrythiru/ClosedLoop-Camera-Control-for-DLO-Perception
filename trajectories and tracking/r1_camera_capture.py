...
# Pose C's hold window is (36.0, 140.0) -- at 30Hz that's ~3000 captures;
# capture stops early at absolute sequence time t=51.5s instead of running
# to the hold's actual end (user-specified). No override for A/B.
CAPTURE_STOP_OVERRIDE: dict[str, float] = {"C": 51.5}
...
class R1CalibCapture:
    ...
    def __init__(
        ...
        # 2026-09-22: open_ended=True (run_dlo_v4_freeze.py's --nbv) skips the
        ...
        # whose end isn't known until it happens. Mirrors the identical
        # addition to r1_camera_capture_v5.py. See maybe_capture_open_ended().
        self.open_ended = open_ended
        self._next_capture_t: dict[str, float] = {}
        self.model = model
        self.r1_cam_arm = r1_cam_arm
        # 2026-09-22: optional live-publish hook (trajectories/
        # r1_live_tracking_bridge_v4.py's LiveTrackingManager) -- None by
        # default, so this file keeps zero ROS dependency and zero
        # behavior change unless run_dlo_v4_freeze.py's --ros-live flag
        # explicitly constructs and passes one in. Mirrors the identical
        # hook added to r1_camera_capture_v5.py for V5.
        ...
        # V5 has 11 -> node 10), so the metadata key "grasp_node19_pos"
    ...
    def maybe_capture_open_ended(
        ...
        end isn't known in advance (run_dlo_v4_freeze.py's --nbv). Mirrors
        r1_camera_capture_v5.py's identical method: ticks a per-label "next
        capture due" time forward by CAPTURE_PERIOD_S after each capture
        instead of comparing against a precomputed instants array. First
        sight of a label lazily creates its output dir and schedules its
        first capture START_MARGIN_S after that moment. Capture stops the
        instant pose_label stops matching -- no END_MARGIN_S/stop_at."""
    ...
    def _do_capture(
        ...
        # 2026-09-22: live-publish, alongside the disk save above -- reuses
        # the SAME rendered rgb/depth_rgb_cam arrays (no duplicate render).
        # depth_rgb_cam (not depth), matching this file's own module
        # docstring: it's r1_d435i_rgb's own co-located depth channel,
        # pixel-aligned with rgb -- the separate r1_d435i_depth sensor has
        # a real baseline offset + different fovy and would misregister.
        # Mirrors the identical hook added to r1_camera_capture_v5.py.
