"""V4 variant of verify_r1_camera_reachability.py -- targets
generate_triple_scene_v4's scene instead of v3's, since v4 moved r1's base
(and its rail) 0.25m further in -Y (see DLO_PLACEMENTS in
generate_triple_scene_v4.py), which invalidates any pose solved against
...
Usage: venv/bin/python3 trajectories/verify_r1_camera_reachability_v4.py

Prints a ready-to-paste `set_solved_waypoints(...)` call at the end --
paste its output into run_dlo_v4_freeze.py as instructed.
"""
...
from r1_camera_ik import (  # noqa: E402
    CameraArm, POSE_A, POSE_B, POSE_C, R1_HOME_QPOS,
    RAIL_A, RAIL_B, RAIL_C,
)
...
def main() -> int:
    ...
    rail_by_label = {"A": RAIL_A, "B": RAIL_B, "C": RAIL_C}

    q_seed = R1_HOME_QPOS.copy()
    solved = {}
    all_reachable = True
    for label, (pos, quat) in (("A", POSE_A), ("B", POSE_B), ("C", POSE_C)):
        data.qpos[rail_qadr] = rail_by_label[label]
        ...
        print(f"[verify] Pose {label}: {verdict}  rail_x={rail_by_label[label]:+.3f}  "
    ...
    print()
    print(f"[verify] {'ALL 3 REACHABLE' if all_reachable else 'SOME POSES UNREACHABLE -- see nearest-reachable qpos above'}")
    print()

    all_clean = True
    all_clean &= _sweep_transition(arm, model, data, rail_qadr, R1_HOME_QPOS, 0.0, solved["A"], rail_by_label["A"], "home->A")
    all_clean &= _sweep_transition(arm, model, data, rail_qadr, solved["A"], rail_by_label["A"], solved["B"], rail_by_label["B"], "A->B")
    all_clean &= _sweep_transition(arm, model, data, rail_qadr, solved["B"], rail_by_label["B"], solved["C"], rail_by_label["C"], "B->C")
    print()
    print(f"[verify] {'ALL TRANSITIONS CLEAN' if all_clean else 'SOME TRANSITIONS COLLIDE -- see above'}")
    print()

    print("[verify] Paste this into run_dlo_v4_freeze.py (call once, before planner.run_sequence()):")
