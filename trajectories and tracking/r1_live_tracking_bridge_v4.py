"""Live (in-process) TrackDLO bridge for V4 -- publishes r1's captured
frames to ROS AS the MuJoCo sim steps, instead of
trackdlo_eval/run_r1_tracking.py's disk-replay-after-the-fact approach.
Mirrors trajectories/r1_live_tracking_bridge.py (V5's live bridge)
exactly, pointed at V4's files -- see OFlinevsONline.md (repo root) for
the full offline-vs-live comparison this module implements, and
LiveTrackDLO_Plan.md/SessionRetention_2026-09-22.md for the design this
mirrors.

2026-09-22: V4's offline pipeline (run_dlo_v4_freeze.py,
trajectories/r1_camera_capture.py, trackdlo_eval/run_r1_tracking.py)
is left completely untouched in behavior -- this is a purely additive
sibling to V5's live bridge, not a replacement of anything. Confirmed
before writing this: V4's run_r1_tracking.py already has 1:1 identical
structure to V5's (same PoseSession/_launch_trackdlo_node/_marker_flip/
_endpoint_flip/bridge_seed functions, same "Option B" fresh-init-every-
pose design already active in its main()) -- only NUM_NODES=20 (not 15),
CAPTURE_HZ=30 (not 15, see r1_camera_capture.py), and 3 poses (A/B/C, not
A/B) differ. None of those differences need special-casing here: this
module reacts to hold-window transitions generically (whatever pose
labels show up), so 2 vs 3 holds needs no code change.

Scope: same as V5's -- r1's camera still follows its existing fixed,
pre-baked schedule; this module only reacts to hold-window transitions,
it doesn't drive them. Node-identity resolution is "Option B": every
hold gets a fresh trackdlo+init_tracker relaunch and a real HSV+skeleton
re-init (no bridge_seed/synthetic-init-nodes position-carrying -- that
approach was tried earlier in this codebase and abandoned, see
run_r1_tracking.py's bridge_seed docstring). Only node ORDER IDENTITY is
chained across holds (via _endpoint_flip against the previous hold's
last live result), never position. No node_support/tracking_diag
(Bucket-3) logging here, matching V5's live bridge.

IMPORTANT: requires rospy/roslaunch (the ros_noetic conda env with
~/tracking_ws sourced), NOT this project's venv. This module is only
ever imported when run_dlo_v4_freeze.py's --ros-live flag is passed, so
it has no import-time effect otherwise.
"""
...
from run_r1_tracking import (  # noqa: E402
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "results" / "r1_dlo_tracking_v4_live"
...
class LiveTrackSession:
    ...
    Identical to V5's LiveTrackSession (trajectories/r1_live_tracking_bridge.py)
    except for importing NUM_NODES=20/etc. from run_r1_tracking (V4) instead
    of run_r1_tracking_v5."""
    ...
    def _on_results(self, msg) -> None:
        ...
        if self._pending_flip_resolution:
            ...
            print(f"[live-track-v4] node order {'REVERSED' if self._flip else 'forward'} ({why})", flush=True)
...
class LiveTrackingManager:
    """Owned by run_dlo_v4_freeze.py's main() -- watches which hold
    ...
    by the viewer's on_frame for the purple marker overlay. Generic over
    the NUMBER of holds -- works for V4's 3 (A/B/C) the same way V5's
    LiveTrackingManager works for 2 (A/B), no special-casing needed."""
    ...
    def on_capture(self, rgb: np.ndarray, depth: np.ndarray, cam_pos: np.ndarray, cam_quat: np.ndarray,
        ...
        """Call once per R1CalibCapture._do_capture (V4's
        trajectories/r1_camera_capture.py) -- i.e. only at CAPTURE_HZ
        (30Hz for V4), only inside a hold window (pose_label is never
        None here; R1CalibCapture already gates that)."""
        import rospy
    ...
    def _enter_hold(self, pose_label: str) -> None:
        ...
        if self._launch is not None:
            ...
            print(f"[live-track-v4] leaving hold {self._current_label}: stopping trackdlo/init_tracker", flush=True)
            self._launch.stop()
        ...
        print(f"[live-track-v4] entering hold {pose_label}: relaunching trackdlo+init_tracker", flush=True)
    ...
    def save_output(self, path: Path | None = None) -> None:
        ...
        print(f"[live-track-v4] saved {len(t_arr)} live-tracked frames to {path}", flush=True)

    def shutdown(self) -> None:
        if self._launch is not None:
            print(f"[live-track-v4] shutting down (last hold: {self._current_label})", flush=True)
            self._launch.stop()
            self._launch = None
