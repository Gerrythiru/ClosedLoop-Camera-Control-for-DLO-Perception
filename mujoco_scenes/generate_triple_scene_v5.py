...
# Custom grasp-tool STL (2026-09-17): mates inside r2/r3's gripper jaws at
# the closed position, giving the cable a known, deterministic mount point
# -- fixes the contact-weld position-offset problem at the grasped ends
# (node0/node14), which isn't reconstructible from any current robot state
# (see Today_SUMM.md, Section 12 item B9 / the node0 "levitation" finding).
# Path is relative to <compiler meshdir>, same convention as
# D435I_MESH_RELDIR below (4 "../" climbs assets -> ufactory_lite6 ->
# mujoco_menagerie -> vendor -> repo root, where the STL actually lives).
GRASP_TOOL_MESH_RELPATH = "../../../../Tool_For_r2,r3_Closed_Gripper_Position.STL"
# STL units are millimeters (measured bbox 22.1 x 22.2 x 10mm -- the
# 22.1/22.2mm footprint matches the measured closed-jaw gap almost
# exactly, confirming both the unit and the snug-fit design intent);
# MuJoCo scene units are meters.
GRASP_TOOL_SCALE = "0.001 0.001 0.001"
# The tool's two flat faces (its bore/cut) are geometrically IDENTICAL
# (checked directly: same outer boundary, same ~6.1mm bore radius, same
# notch feature, at both mesh-local Z=0mm and Z=10mm) -- there is no
# geometric way to tell "bottom" from the mesh alone, only from the CAD
# author's own modeling convention. Mounted here with mesh-local Z=0 (the
# STL's own origin corner) toward the gripper body and Z=10mm (mesh's far
# end) toward the fingertips/grasp point -- i.e. the far end is what's
# being called "downward-facing" per the user's description. Tool-local
# Y (22.2mm, matching the jaw gap) is mounted aligned with the gripper's
# own jaw-closing axis (finger joint axis="0 1 0"); tool-local X/Z map
# directly to gripper-local X/Z (identity quat, no rotation applied).
# FIRST-PASS ESTIMATE, not yet visually verified -- if the mesh appears
# rotated 90 deg (X/Y swapped) or needs to sit further from/closer to the
# gripper body, adjust GRASP_TOOL_LOCAL_POS/_QUAT below; flipping which
# end faces "down" makes no visible difference since both ends are
# identical.
GRASP_TOOL_LOCAL_QUAT = "0.5 -0.5 -0.5 0.5"  # 2026-09-17: user-set, visually verified correct
# The STL's own origin is a CORNER of its bounding box (measured: X in
# [0,22.1mm], Y in [0,22.2mm], Z in [0,10mm]), not its center.
_GRASP_TOOL_RAW_EXTENT_M = (0.0221, 0.0222, 0.010)
# lite6_gripper_wide.xml's own `end_effector` site, pos="0 0 0.0811"
# (gripper_body-local), documented there as "the centre at the tip of the
# grippers" -- the authoritative "fingers height" reference.
_GRIPPER_FINGERTIP_Z = 0.0811
# 2026-09-17: tool moved from "top face flush against the housing ceiling"
# (pushed as far up/proximal as possible) to "bottom face (the circular
# cut, facing away from joint6/toward the grasp point) sits 1cm PAST the
# fingertips" -- so the tool's hole becomes the cable's only real contact
# point; the fingers themselves can close without ever touching the cable.
_GRASP_TOOL_BOTTOM_FACE_Z = _GRIPPER_FINGERTIP_Z + 0.01


def _solve_grasp_tool_pos(quat_wxyz: str, target_z: float, anchor_face: str) -> tuple[float, float, float]:
    """Body pos (gripper_body-local) that (a) centers the mesh's raw
    bounding-box center at gripper-local X=0, Y=0 -- the jaw centerline,
    same for both open and closed since both fingers start symmetric
    about it -- and (b) places whichever mesh face CURRENTLY has its
    outward normal pointing toward gripper-local +Z ("down, away from
    joint6, toward the grasp point") if anchor_face="bottom", or -Z ("up,
    toward joint6") if anchor_face="top", at target_z. Re-derived from the
    CURRENT quat every call rather than a hand-computed constant, so
    re-rotating GRASP_TOOL_LOCAL_QUAT doesn't silently leave a stale
    position behind."""
    q = np.array([float(v) for v in quat_wxyz.split()])
    mat = np.zeros(9)
    mujoco.mju_quat2Mat(mat, q)
    R = mat.reshape(3, 3)

    ex, ey, ez = _GRASP_TOOL_RAW_EXTENT_M
    raw_center = np.array([ex / 2, ey / 2, ez / 2])

    gripper_dir = np.array([0.0, 0.0, 1.0 if anchor_face == "bottom" else -1.0])
    n_mesh = R.T @ gripper_dir
    axis = int(np.argmax(np.abs(n_mesh)))
    p_face = raw_center.copy()
    p_face[axis] = _GRASP_TOOL_RAW_EXTENT_M[axis] if n_mesh[axis] > 0 else 0.0

    center_gripper = R @ raw_center
    face_gripper = R @ p_face
    return (
        float(-center_gripper[0]),
        float(-center_gripper[1]),
        float(target_z - face_gripper[2]),
    )


GRASP_TOOL_LOCAL_POS = _solve_grasp_tool_pos(
    GRASP_TOOL_LOCAL_QUAT, _GRASP_TOOL_BOTTOM_FACE_Z, anchor_face="bottom"
)
OUTPUT_NAME = "triple_lite6_cable_routing.xml"
DLO_OUTPUT_NAME = "triple_lite6_cable_routing_dlo_v5.xml"
CABLE_DIAMETER = 0.010
CABLE_RADIUS = CABLE_DIAMETER / 2.0
CABLE_SEGMENT_LENGTH = 0.04
# V5: 15 segments (was 20). Segment length kept at 0.04 m so the capsule
# aspect ratio (4:1) and node spacing are unchanged; the cable is simply
# 75% as long (15 x 0.04 = 0.60 m tip-to-tip vs 0.80 m), trimmed equally
# about its midpoint (world x = 0.18).
CABLE_SEGMENTS = 15
...
def add_routing_box(worldbody: ET.Element) -> None:
    ...
    add_geom(box, name="box_floor", type="box", pos="0 0 -0.038", size="0.2325 0.15 0.012", material="mat_box")
    # V5: box length scaled to 75% about its center (body local x = 0), i.e.
    # every local-X coord and X half-size below is 0.75 x the original. Width
    # (y), height (z) and the two extrude-cut gaps' proportions are unchanged.
    # Original (x0.75 -> current):
    #   floor half-x   0.31   -> 0.2325
    #   guide A center -0.21  -> -0.1575    half 0.10   -> 0.075
    #   guide B center  0.07  ->  0.0525    half 0.07   -> 0.0525
    #   guide C center  0.265 ->  0.19875   half 0.015  -> 0.01125
    # (The 4 unused box_inlet/outlet_{red,blue} visual sites -- two-cable
    #  routing markers from the original layout -- were dropped in V5.)
    for y, tag in ((-0.105, "front"), (0.105, "back")):
        add_geom(box, name=f"box_{tag}_guide_a", type="box", pos=f"-0.1575 {y} 0.005",  size="0.075 0.010 0.045",    material="mat_box")
        add_geom(box, name=f"box_{tag}_guide_b", type="box", pos=f"0.0525 {y} 0.005",   size="0.0525 0.010 0.045",   material="mat_box")
        add_geom(box, name=f"box_{tag}_guide_c", type="box", pos=f"0.19875 {y} 0.005",  size="0.01125 0.010 0.045",  material="mat_box")
...
def add_cable_grasp_weld(
    ...
    grippers (v5: both ends are grasped -- r2 takes node 0, r3 takes the
    last node).
...
#
# v5: r1 (and its rail) pushed another 0.25m in -Y, from -0.27 to -0.52, to
...
# run_dlo_v5_freeze.py) were re-solved after this move -- the old ones are
...
# v5: r1 gets an actuated horizontal linear rail, sliding its whole mount
...
def add_assets(root: ET.Element, source_asset: ET.Element | None) -> ET.Element:
    ...
    ET.SubElement(asset, "mesh", name="grasp_tool", file=GRASP_TOOL_MESH_RELPATH, scale=GRASP_TOOL_SCALE)
    ET.SubElement(asset, "material", name="mat_grasp_tool", rgba="0.85 0.65 0.13 1")
...
def add_grasp_tools(worldbody: ET.Element) -> None:
    """Mounts the grasp_tool mesh (see GRASP_TOOL_* constants above)
    rigidly inside r2's and r3's gripper jaws -- no new joint, it moves
    with the gripper body directly, matching how it's physically mated
    (contact-fit, not actuated). r1 (the wrist camera arm, never grasps
    anything) is deliberately excluded."""
    for prefix in ("r2_", "r3_"):
        gripper_body = find_body(worldbody, f"{prefix}gripper_body")
        if gripper_body is None:
            raise ValueError(f"Could not find {prefix}gripper_body in worldbody tree.")
        tool_body = ET.SubElement(
            gripper_body, "body", name=f"{prefix}grasp_tool",
            pos=text_vec(GRASP_TOOL_LOCAL_POS), quat=GRASP_TOOL_LOCAL_QUAT,
        )
        add_geom(
            tool_body, name=f"{prefix}grasp_tool_geom",
            type="mesh", mesh="grasp_tool", material="mat_grasp_tool",
            contype="1", conaffinity="1", density="1200",
        )
...
def generate(
    ...
    model_name = "triple_lite6_cable_routing_dlo_v5" if layout == "dlo" else "triple_lite6_cable_routing"
    ...
    if layout == "default":
        ...
        # V5: start_pos x shifted -0.20 -> -0.10 so the (now 15-segment, 0.60 m)
        # cable keeps its midpoint at world x = 0.18
        # (start_x + (segments-1)/2 * CABLE_SEGMENT_LENGTH = -0.10 + 7*0.04 = 0.18).
        add_physical_cable(worldbody, "red", "mat_cable_red", 0,
                           segments=segments, radius=radius, start_pos=(-0.10, 0, 0.845),
