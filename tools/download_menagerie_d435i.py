...
def download_d435i(force: bool = False) -> Path:
    root = project_root()
    ...
    print(f"Downloaded {len(files)} D435i files to {vendor_model}")
    return vendor_model


def main() -> int:
    parser = argparse.ArgumentParser(description="Download official Intel RealSense D435i MuJoCo Menagerie files.")
    parser.add_argument("--force", action="store_true", help="Overwrite files in the vendor directory.")
    args = parser.parse_args()

    try:
        download_d435i(force=args.force)
