...
The chained V4 trajectory (results/r1_dlo_tracking_v4/combined_tracked_trajectory.npz)
...
def _bridge_gap(pre, post, t0, t1, bk, dt):
    ...
    pre-gap and first post-gap tracked frames (20, 3).
    ...
    for x in times:
        ...
        out.append(f)
    return times, np.array(out).reshape(-1, 20, 3), float(np.linalg.norm(resid0)), float(np.linalg.norm(resid19))
...
def main() -> int:
    ...
    if nodes.shape[1:] != (20, 3):
        raise SystemExit(f"expected (T, 20, 3) nodes, got {nodes.shape}")
    ...
    if not np.isfinite(bk["grasp_offset_0"]).all() or not np.isfinite(bk["grasp_offset_19"]).all():
        raise SystemExit("bridge_kinematics.npz has no settled grasp offset -- re-run dump_v4_ground_truth.py --full")
