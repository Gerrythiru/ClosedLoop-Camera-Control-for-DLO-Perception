...
Inputs (per pose P in A,B,C):
  results/r1_dlo_tracking_v5/combined_tracked_trajectory_{P}.npz  (t, nodes)
  results/r1_eye_in_hand_calib_v5/pose_{P}/ground_truth.npz       (t, gt_nodes, occlusion)

Both are world/base frame, N nodes (15 in V5), matched exactly by capture time `t`.
...
Outputs:
  results/r1_dlo_tracking_v5/stitch_3d_gt_vs_tracked.html   (plotly: rotate/zoom + time slider)
  results/r1_dlo_tracking_v5/stitch_3d_gt_vs_tracked.gif    (matplotlib: fixed iso view)
...
def _load_pose(pose: str):
    ...
    t_trk, nodes = trk["t"], trk["nodes"]                      # (T,), (T,N,3)
...
def _pose_at(t: float) -> str:
    return "A" if t < 37.0 else "B"   # TEMP: A hold [2,36], A->B xfer [36,38], B hold [38,90]; no C
...
def _assemble_chained(tracked_path=None):
    ...
    trk_npz = np.load(tracked_path or TRACK_ROOT / "combined_tracked_trajectory.npz")
...
def main() -> int:
    ...
    ap.add_argument("--tracked", type=Path, default=None,
                    help="--chained only: use this tracked npz instead of the default "
                         "combined_tracked_trajectory.npz (e.g. a sweep run's "
                         "combined_tracked_trajectory_<suffix>.npz). Output filename gets "
                         "the tracked file's own suffix appended so it doesn't clobber the "
                         "default chained stitch.")
    args = ap.parse_args()

    if args.bridged:
        rows, suffix, kind = _assemble_bridged(), "_bridged", "bridged"
    elif args.chained:
        rows = _assemble_chained(args.tracked)
        suffix = f"_chained_{args.tracked.stem.replace('combined_tracked_trajectory', '').lstrip('_')}" if args.tracked else "_chained"
        kind = "chained"
    else:
